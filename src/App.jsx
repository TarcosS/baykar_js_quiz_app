...
function App() {
  ...
  const [{ trues, falses, empty }, setResult] = useState({
    trues: 0,
    falses: 0,
    empty: 0,
  });
  ...
  const updateQuestions = async () => {
    ...
    currentQuestions.map((question) => {
      ...
      for (let i = 0; i < 4; i++) {
        let answer =
          question.body.split(" ")[
            Math.round(Math.random() * (question.body.split(" ").length - 1))
          ];
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow:
            "0px 25px 50px -12px rgba(0,0,0,0.25), 0px 0px 15px 0px rgba(0,0,0,0.07)",
        }}
      >
        <div className="text-white d-flex justify-content-center rounded-top-3 align-items-center flex-column position-relative overflow-hidden">
          <div
          ...
          <span style={{ mixBlendMode: "difference" }}>
            {isTestEnded
              ? `${trues} Doğru - ${falses} Yanlış - ${empty} Boş`
              : second + "sn"}
          </span>
        </div>
        <div
            ...
            <>
              ...
              >
                {questions[questionIndex]?.answers.map((answer, index) => (
                  <div key={"questionIndex_" + index} className="col-sm-6 p-2">
                    <button
                      key={index}
                      className="p-2"
                      style={{
                        ...
                        background:
                          selectedAnswer === index ? "#9cc6ff" : "#f8f9fa",
